...
func Resize(buf []byte, o Options) ([]byte, error) {
	...
	if o.Box {
			inWidthHeightRatio := float64(inWidth) / float64(inHeight);
			outWidthHeightRatio := float64(o.Width) / float64(o.Height);
			if outWidthHeightRatio < inWidthHeightRatio {
					// Width is the limiting factor
					o.Height = int( float64(inHeight) * float64(o.Width) / float64(inWidth) )
			} else {
					o.Width = int( float64(inWidth) * float64(o.Height) / float64(inHeight) )
			}
			o.Force = true
	}
